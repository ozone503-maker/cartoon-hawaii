...
import { hu, latLonToWorld, terrainY, WORLD, WORLD_SCALE, wu } from "@/lib/hawaii/world";
...
export function Locations() {
  return (
    <group>
      ...
      <Sand lat={19.9919} lon={-155.8244} color="#f3ead2" rx={wu(0.7)} rz={wu(0.32)} />
...
function PlaceLabel({ lat, lon, name }: { lat: number; lon: number; name: string }) {
  ...
  useFrame(() => {
    ...
    const near = dx * dx + dy * dy + dz * dz < wu(22) * wu(22);
    if (near !== show) setShow(near);
  });
  if (!show) return null;
  return (
    <Html position={[x, y + hu(1.15), z]} center distanceFactor={28} zIndexRange={[4, 8]} style={{ pointerEvents: "none" }}>
...
function Runway({ lat, lon, len, heading }: { lat: number; lon: number; len: number; heading: number }) {
  ...
  return (
    <group position={[x, y + hu(0.06), z]} rotation={[0, -rad, 0]}>
      <mesh>
        <boxGeometry args={[w, hu(0.04), wu(0.42)]} />
        <meshStandardMaterial color="#3a3e44" roughness={0.85} />
      </mesh>
      <mesh position={[0, 0.03, 0]}>
        <boxGeometry args={[w * 0.92, hu(0.01), wu(0.035)]} />
...
function Sand({ lat, lon, color, rx, rz }: { lat: number; lon: number; color: string; rx: number; rz: number }) {
  ...
  const y = terrainY(x, z);
  return (
    <mesh position={[x, y + hu(0.05), z]} rotation={[-Math.PI / 2, 0, 0.2]} scale={[rx, rz, 1]}>
...
function Taro({ lat, lon }: { lat: number; lon: number }) {
  ...
  const y = terrainY(x, z);
  return (
    <group position={[x, y + hu(0.04), z]} scale={WORLD_SCALE}>
      {[-0.35, 0, 0.35].map((ox) => (
...
function Pier({ lat, lon, heading }: { lat: number; lon: number; heading: number }) {
  ...
  const rad = (heading * Math.PI) / 180;
  return (
    <mesh position={[x, y + hu(0.08), z]} rotation={[0, -rad, 0]}>
      <boxGeometry args={[wu(0.9), hu(0.06), wu(0.18)]} />
...
function Wall({ lat, lon }: { lat: number; lon: number }) {
  ...
  const y = terrainY(x, z);
  return (
    <mesh position={[x, y + hu(0.12), z]} rotation={[-Math.PI / 2, 0, 0]}>
      <ringGeometry args={[wu(0.35), wu(0.48), 16, 1, 0, Math.PI * 1.2]} />
