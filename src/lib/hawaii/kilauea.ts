import { GROUND_SCALE, hu, latLonToWorld, terrainY, worldToLatLon, wu } from "./world";

const CAL = latLonToWorld(19.4069, -155.2834);
const PIT = latLonToWorld(19.405, -155.291);

/**
 * ~4 × 3 km caldera. Radii were authored with wu() when the island was 960 wide.
 * The ground grew ×4 and these did not, so the bowl became a ~400 m pothole.
 */
export const KILAUEA_RX = wu(3.1) * GROUND_SCALE;
export const KILAUEA_RZ = wu(2.2) * GROUND_SCALE;
export const PIT_R = wu(1.15) * GROUND_SCALE;
...
export function kilaueaBowlY(x: number, z: number, y0: number) {
  ...
  let drop = t * hu(0.7) * GROUND_SCALE;
  const px = (x - PIT.x) / PIT_R;
  const pz = (z - PIT.z) / PIT_R;
  const pe = px * px + pz * pz;
  if (pe < 1) drop += Math.min(1, (1 - pe) / 0.35) * hu(0.55) * GROUND_SCALE;
  return y0 - drop;
}
